<template>
  <el-dialog title="添加阶段成员" :visible.sync="innerVisible" :close-on-click-modal="false" width="60%" class="add-organize">
    <el-row>
      <el-row>
        <el-col :span="24">
...
export default {
  ...
  methods: {
    ...
    getDataList () {
      let thisvm = this
      // 只查询测试组成员
      let params = { 'stageMemberType': '2', ...thisvm.dataForm }
    ...
    handleConfirm () {
      ...
      for (let key in projectMemberDetailMap) {
        if ({}.hasOwnProperty.call(projectMemberDetailMap, key)) {
          ...
          item.projectStageId = thisvm.projectStageId
          item.stageId = thisvm.stageId
          data.push(item)
        }
      }
      if (data.length > 0) {
