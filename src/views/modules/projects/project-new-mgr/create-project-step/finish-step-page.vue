<template>
  <el-row class="finish-step" v-if="projectData.readOnly==0">
    <el-col :span="24" style="text-align:center;margin-top:15%;">
      <el-button type="primary" round @click="doStartProjectStage" v-if="projectData.type=='0' &&  showStageBtn">启动任务</el-button>
      <el-button type="primary" round @click="doCreate">{{createBtnTxt}}</el-button>
...
export default {
  ...
  computed: {},
  mounted () {
    if (this.projectData.type === '0') {
      this.createBtnTxt = '创建任务'
    } else {
      this.createBtnTxt = '创建子项目'
    }
  },
  methods: {
    ...
    doCreate () {
      ...
      if (type === '0') {
        let title = '新建任务'
        let params = { 'parentId': '', 'projectId': projectId || '', 'projectName': projectName, 'type': type }
        // 组装当前路径参数
        ...
        let title = '创建子项目'
        let params = { 'parentId': projectId, 'projectName': projectName }
        // 组装当前路径参数
        let pagePathParams = { 'pageName': '项目管理', 'id': '', 'type': '' }
