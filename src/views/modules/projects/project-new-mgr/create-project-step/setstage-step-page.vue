<template>
  <el-row>
    <el-col :span="14" class="split-line" style="text-align:left;">
      阶段列表 <span style="color:#4190f7;padding-left:24px;">说明：因为任务要在各阶段下创建，所以请先设置当前项目涉及到的阶段信息</span>
    </el-col>
    <el-col :span="10" style="text-align:right;" v-if="projectData.readOnly==0">
    ...
    <el-col :span="24" style="margin-top:24px">
      <el-table stripe :data="stageTableData" border style="width: 100%;height:500px;overflow-y:auto;">
    ...
    <add-stage-review-member :visible.sync="addStageReviewMember" ref="addStageReviewMember" @watchChildMethod="triggerParentMethod"></add-stage-review-member>
  </el-row>
</template>
<script>
...
import AddStageReviewMember from './choose-dialog/add-stage-review-member'
export default {
  name: 'SetstageStepPage',
  components: {
    addStage, addStageTemplate, addStageMember, AddStageReviewMember
  },
  props: {
    projectData: Object
  },
  ...
  methods: {
    ...
    handleAddMemberClick (row) {
      // 特殊处理关于阶段评审，可以选所有用户
      // let refKey = ['2', '4', '6'].includes(row.stageId) ? 'addStageReviewMember' : 'addStageMember'
      let refKey = 'addStageMember'
      this.$refs[refKey].projectId = this.projectData.projectId
      this.$refs[refKey].projectStageId = row.projectStageId
      this.$refs[refKey].stageId = row.stageId
      this.$refs[refKey].init()
    },
    handleEditClick (row) {
