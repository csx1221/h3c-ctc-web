...
export default {
  ...
  methods: {
    ...
    getOrganizeTreeData () {
      ...
      params.parentId = thisvm.parentId
      // 根据是否查询，来判断是否展开
      let expandedBool = (params.name !== '')
      thisvm.$http.get('/ctc/project/projectParticipant/subProjectSelectableParticipant', { 'params': params }).then(({ data: res }) => {
        if (res.code !== 0) {
