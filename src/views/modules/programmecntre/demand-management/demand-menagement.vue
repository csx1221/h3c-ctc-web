    ...
    <el-col :span="24">
      <el-card shadow="never" class="aui-card--fill">
        <el-form label-position="right" :inline="true" ref="dataForm" :model="dataForm" label-width="100px">
          <el-row>
            <el-col :span="5">
              <el-form-item label="需求分析编号" prop="analyzeCode">
                <el-input v-model="dataForm.analyzeCode" placeholder="请输入" clearable></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="5">
              <el-form-item label="分析人" prop="userId">
            ...
            <el-col :span="5">
              <el-form-item label="子项目" prop="projectId">
                ...
                <!--            <ctc-select v-model="dataForm.projectId" dict-type="project.scope" placeholder="请选择"></ctc-select>-->
              </el-form-item>
            </el-col>
      ...
      <el-card shadow="never" class="aui-card--fill">
        ...
        <el-table stripe v-loading="dataListLoading" :data="dataList" border @selection-change="dataListSelectionChangeHandle" @sort-change="dataListSortChangeHandle" style="width: 100%;">
          ...
          <template v-for="(item, index) in titleDataList">
            <el-table-column v-if="item.code==='analyzeCode'" :key="index" :label="item.name" align="center">
...
export default {
  ...
  methods: {
    ...
    getProjectInfo (val) {
      ...
      this.$http.get('/ctc/plan/analyze/list', { 'params': params }).then(({ data: res }) => {
    ...
    viewDetails (row) {
      ...
      this.$nextTick(() => {
        this.$refs.addOrUpdataOrLook.init('demandMenagement', 'look', row)
      })
    },
    doTableOperate (type, row) {
      if (type === 0) { // 编辑
        this.addOrUpdataOrLookVisible = true
        this.$nextTick(() => {
          this.$refs.addOrUpdataOrLook.init('demandMenagement', 'update', row)
        })
      } else if (type === 1) { // 删除
        this.dels(row.analyzeId)
      } else if (type === 2) {
        this.locus(row.analyzeId)
      }
    },
    createConfig () {
    ...
    dels (id) {
      this.$confirm('此操作将永久删除, 是否继续?', '提示', {
        ...
        this.$http.delete('/ctc/plan/analyze/' + id, {}).then(({ data: res }) => {
    ...
    locus (id) {
      this.$refs.caseTrajectory.init('/ctc/plan/analyze/')
