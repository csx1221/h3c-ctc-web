  ...
  methods: {
    init () {
      ...
      this.$nextTick(() => {
        ...
        if (this.dataForm.id !== '' && this.dataForm.id !== undefined) {
          setTimeout(function () { thisvm.getDeptInfo(thisvm.dataForm.id) }, 400)
        }
      })
    },
    // 获取信息
    getDeptInfo (id) {
      this.$http.get(`/sys/deptnew/${this.dataForm.id}`).then(({ data: res }) => {
        if (res.code !== 0) {
    ...
    dataFormSubmitHandle: debounce(function () {
      this.$refs['dataForm'].validate((valid) => {
        ...
        this.$http[!this.dataForm.id ? 'post' : 'put']('/sys/deptnew', this.dataForm).then(({ data: res }) => {
          if (res.code !== 0) {
            return this.$message.error(res.msg)
          }
          this.$message({
