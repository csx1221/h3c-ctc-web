<template>
  <el-card shadow="never" class="aui-card--fill">
    <div class="mod-sys__region">
      ...
      <el-table stripe highlight-current-row v-loading="dataListLoading" :data="dataList" row-key="id" border lazy :load="load" style="width: 100%" :tree-props="{children: 'children', hasChildren: 'hasChildren'}">
        <el-table-column prop="name" :label="$t('region.name')" header-align="center" align="center" min-width="180">
          <template slot-scope="scope">
            {{ scope.row.name }}
          </template>
        </el-table-column>
        <el-table-column prop="id" :label="$t('region.id')" header-align="center" align="center" min-width="180">
          <template slot-scope="scope">
            {{ scope.row.id }}
          </template>
        </el-table-column>
        <el-table-column prop="treeLevel" :label="$t('region.type')" header-align="center" align="center" min-width="150">
          <template slot-scope="scope">
