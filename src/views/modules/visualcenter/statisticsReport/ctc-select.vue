<template>
  <el-select :value="multiple?value:value+''" @input="$emit('input', $event)" :disabled="disabled" :multiple="multiple" :placeholder="placeholder" clearable>
    <el-option :label="data.dictLabel||data.state" v-for="data in dataList" :key="data.dictValue||data.code" :value="data.dictLabel||data.state">{{data.dictLabel||data.state}}</el-option>
  </el-select>
</template>
<script>
