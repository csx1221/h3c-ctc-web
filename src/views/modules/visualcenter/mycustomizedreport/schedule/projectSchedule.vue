        ...
        <el-table stripe highlight-current-row v-loading="dataListLoading" :data="dataList" border @row-click="dataListClick" @selection-change="dataListSelectionChangeHandle" style="width: 100%;">
            ...
            <el-table-column prop="projectStatus" label="项目状态" header-align="center" align="center" show-overflow-tooltip>
                <template slot-scope="scope">
                    <span v-if="scope.row.projectStatus === -1">未配置完成</span>
                    <span v-else-if="scope.row.projectStatus === 0">未开始</span>
                    <span v-else-if="scope.row.projectStatus === 1">正常在测</span>
                    <span v-else-if="scope.row.projectStatus === 2">正常完成</span>
                    <span v-else-if="scope.row.projectStatus === 3">延期在测</span>
                    <span v-else-if="scope.row.projectStatus === 4">延期完成</span>
                    <span v-else>{{scope.row.projectStatus}}</span>
                </template>
            </el-table-column>
            <el-table-column prop="progressRateDTO.actualProgress" label="实际进度" header-align="center" align="center" show-overflow-tooltip></el-table-column>
...
export default {
  ...
  data () {
    return {
      mixinViewModuleOptions: {
        getDataListURL: '/ctc/visualization/report/project/progress',
  ...
  methods: {
    ...
    doExportData () {
      ...
      let url = '/ctc/visualization/report/project/progress/export'
