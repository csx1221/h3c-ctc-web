...
<!--                      type 1 批量申请 2批量变更-->
                       <account-batch ref="accountBatch" :type="'1'" :data.sync="dataForm.applyList"></account-batch>
                    </el-form-item>
                </el-col>
                <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
...
export default {
  ...
  methods: {
    ...
    submitData (val) {
      ...
      if (this.$route.params.type === '1') {
        url = '/account4a/apply/account/create/batch'
      ...
      thisvm.$http.get('/account4a/apply/account/info', { 'params': params }).then(({ data: res }) => {
        ...
        if (res.data) {
          ...
          for (let i = 0; i < newApplyList.length; i++) {
            ...
            node.cardValue = newApplyList[i].idCard
            ...
            node.organId = newApplyList[i].organizeFrom.serviceId
            node.organName = newApplyList[i].organName
            node.organizeFrom = { organId: newApplyList[i].organId }
            //
            let serviceIdList = newApplyList[i].serviceOrgList
            let serviceId = []
