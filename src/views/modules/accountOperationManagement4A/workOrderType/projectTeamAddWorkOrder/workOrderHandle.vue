        ...
        <el-row>
            ...
            <el-form :model="dataForm" :rules="dataRule" ref="dataForm" label-width="180px">
                ...
                <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
                    <el-form-item label="项目组信息：">
                        <el-row class="project-mgr-page account-4a">
                            <el-col :span="24">
                                <el-card shadow="never" class="aui-card--fill">
                                    <el-table max-height="450px" stripe highlight-current-row :data="echoData.accountList" row-key="id" border default-expand-all :tree-props="{children: 'children', hasChildren: 'hasChildren'}" style="width: 100%;">
                                        ...
                                        <template v-for="(item, index) in accountTitleDataList">
                                            <el-table-column :key="index" :label="item.name" align="left" show-overflow-tooltip>
  ...
  methods: {
    getEchoData (id) {
      ...
      thisvm.$http.get('/account4a/apply/orgainze/info', { 'params': params }).then(({ data: res }) => {
        ...
        let node = {}
        node.createDate = newData.createDate
        node.fileId = newData.fileId
        node.id = newData.id
        node.reason = newData.reason
        node.state = newData.state
        node.title = newData.title
        node.type = newData.type
        node.userName = newData.userName
        node.workNo = newData.workNo
        node.accountList = [
          {
            // hasChildren: true,
            id: '0',
            ...
            children: [
              {
                id: '11',
                lv: 1,
                orgDesc: newData.toCreateOrganizeForm.parentDesc,
                orgName: newData.toCreateOrganizeForm.parentName,
                parentId: '0',
                children: [
                  {
                    id: '22',
                    lv: 2,
                    orgDesc: newData.toCreateOrganizeForm.orgDesc,
                    orgName: newData.toCreateOrganizeForm.orgName,
                    parentId: '11'
                  }
                ]
              }
