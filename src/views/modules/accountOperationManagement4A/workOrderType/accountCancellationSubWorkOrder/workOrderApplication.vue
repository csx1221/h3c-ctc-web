                        ...
                        <el-row class="project-mgr-page account-4a">
                            <el-col :span="24">
                                <el-card shadow="never" class="aui-card--fill">
                                    ...
                                    <el-table stripe v-loading="dataListLoading" :data="dataForm.accountList" border @selection-change="dataListSelectionChangeHandle" @sort-change="dataListSortChangeHandle" style="width: 100%;">
...
export default {
  ...
  methods: {
    ...
    getProjectInfo () {
      ...
      this.$http.get('/account4a/ctc/cancel/account/getCtcAccount', { 'params': params }).then(({ data: res }) => {
        ...
        if (res.code !== 0) {
          ...
          if (list !== null && list.length !== 0) {
            this.dataForm.accountList = this.dataForm.accountList.concat(list)
          }
    ...
    judgeState () {
      ...
      let subData = thisvm.dataForm.accountList
      thisvm.visible = true
      thisvm.$nextTick(() => {
    ...
    submitData (data) {
      ...
      if (data) {
        ...
        node.subAccountCancelFormList = data.accountList
        node.mangerUpdateList = data.mangerUpdateList
      }
      if (this.$route.params.type === '1') {
        url = '/account4a/ctc/cancel/account/create'
        params = { ...node }
      } else if (this.$route.params.type === '2') {
        url = '/account4a/ctc/cancel/account/edit'
    ...
    getEchoData (id) {
      ...
      thisvm.$http.get('/account4a/ctc/cancel/account/info', { 'params': params }).then(({ data: res }) => {
        ...
        if (res.data) {
          ...
          this.dataForm.accountList = data.ctcAccountList
