                        ...
                        <el-row class="project-mgr-page account-4a">
                            <el-col :span="24">
                                <el-card shadow="never" class="aui-card--fill">
                                    <el-table max-height="450px" stripe :data="echoData.accountList" border style="width: 100%;">
  ...
  methods: {
    getEchoData (id) {
      ...
      thisvm.$http.get('/account4a/ctc/org/apply/info', { 'params': params }).then(({ data: res }) => {
        ...
        thisvm.echoData.accountList = [res.data.infoDTO]
      }).catch(() => {
        thisvm.$ctcLoading.close()
      })
    },
    ...
    getTableColumnData (row, item) {
      let columnValue = ''
      if (item.code === 'type') {
        columnValue = this.$getDictLabel('organizeType', row[item.code])
      } else {
