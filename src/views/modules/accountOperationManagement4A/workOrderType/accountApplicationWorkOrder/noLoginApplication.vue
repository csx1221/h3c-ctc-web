...
<!--                  {{dataForm}}-->
                  <el-form :model="dataForm.applyList" :rules="dataRules" ref="dataForm_child" label-width="180px">
                    ...
                    <template v-if="dataForm.applyList.type==='0'">
                      ...
                      <el-col :xs="24" :sm="24" :md="24" :lg="12" :xl="12">
<!--                        {{dataForm.toCreateOrganize}}-->
                        <el-form-item class="projectNew" label="部门/项目组：" prop="orgNew">
                          <el-input v-model="dataForm.applyList.orgNew" style="display: none"></el-input>
                          <ctc-project-team v-if="dataForm.isAddOrg==='0'" v-model="dataForm.applyList.orgId"></ctc-project-team>
                          <project-group-add v-model="dataForm.toCreateOrganizeForm"></project-group-add>
                        </el-form-item>
                      </el-col>
                      <el-col :xs="24" :sm="24" :md="24" :lg="12" :xl="12">
                    ...
                    <el-col :xs="24" :sm="24" :md="24" :lg="12" :xl="12">
                      <el-form-item label="账号角色：" prop="roleIds">
                        <ctc-select-role v-model="dataForm.applyList.roleIds" :type="0" :pid="dataForm.applyList.organization" multiple placeholder="请选择"></ctc-select-role>
                      </el-form-item>
                    </el-col>
                    <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
        ...
        <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
          <el-form-item label="申请人信息：" prop="">
<!--            <el-input disabled v-model="name" placeholder="请输入" clearable></el-input>-->
            <el-row class="project-mgr-page account-4a noLoginForm">
              <el-card shadow="never" class="aui-card--fill">
                <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="12">
                  <el-form-item label="申请人：" prop="applicant">
                    <el-input v-model="dataForm.applicant" clearable></el-input>
                  </el-form-item>
                </el-col>
                <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="12">
                  <el-form-item label="邮箱：" prop="email">
                ...
                <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="12">
                  <el-form-item label="工作单位：" prop="placeOfWork">
                    <el-input v-model="dataForm.placeOfWork" clearable></el-input>
                  </el-form-item>
                </el-col>
              </el-card>
            </el-row>
...
<!--        </el-col>-->
        <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
          <el-form-item label="下一步处理人：" prop="handleUserId">
            ...
            <ctc-next-step-handler ref="handleUser" v-model="dataForm.handleUserId" urlType="6" placeholder="请选择"></ctc-next-step-handler>
<!--              <el-input v-model="dataForm.handleUserId" clearable></el-input>-->
          </el-form-item>
        </el-col>
      </el-form>
    </el-row>
...
import projectGroupAdd from '../../pubilcModule/projectGroupAdd'
export default {
  ...
  watch: {
    ...
    'dataForm.applyList.type' (newVal, oldVal) {
      ...
      this.dataForm.isAddOrg = '0'
      this.dataForm.toCreateOrganizeForm = ''
      this.dataForm.applyList.orgNew = ''
      ...
      this.$refs['dataForm_child'].clearValidate(['orgNew'])
    },
    // 所属机构
    ...
    // 项目组
    'dataForm.applyList.orgId' (newVal, oldVal) {
      this.checkData()
      if (newVal.length > 0) {
        console.log(3)
        this.dataForm.applyList.orgNew = 'true'
      } else {
        console.log(4)
        this.dataForm.applyList.orgNew = ''
      }
    ...
    // 项目组新增
    'dataForm.toCreateOrganizeForm' (newVal, oldVal) {
      console.log(newVal)
      this.dataForm.applyList.orgId = []
      setTimeout(() => {
        if (newVal) {
          console.log(1)
          this.dataForm.isAddOrg = '1'
          // this.dataForm.applyList.orgId = []
          this.dataForm.applyList.orgNew = 'true'
        } else {
          console.log(2)
  ...
  computed: {
    ...
    dataRules () {
      ...
      return {
        ...
        orgNew: [
          { required: true, message: '请选择或者新增项目组', trigger: 'change' },
          { required: true, message: '请选择或者新增项目组', trigger: 'blur' }
        ],
        sysAcctType: [
  ...
  methods: {
    checkData () {
      ...
      let orgId, organizeId, deptId, sectionId
      if (dataForm.isAddOrg === '0') {
        if (dataForm.applyList.orgId && dataForm.applyList.orgId.length > 0) {
          orgId = Array.isArray(dataForm.applyList.orgId) ? dataForm.applyList.orgId[dataForm.applyList.orgId.length - 1] : dataForm.applyList.orgId
          organizeId = dataForm.applyList.organization[0] || ''
          deptId = dataForm.applyList.organization[1] || ''
          sectionId = dataForm.applyList.organization[2] || ''
        ...
        if (dataForm.applyList.organization && dataForm.applyList.organization.length > 0) {
          orgId = Array.isArray(dataForm.applyList.orgId) ? dataForm.applyList.orgId[dataForm.applyList.orgId.length - 1] : dataForm.applyList.orgId
          organizeId = dataForm.applyList.organization[0] || ''
          deptId = dataForm.applyList.organization[1] || ''
          sectionId = dataForm.applyList.organization[2] || ''
    ...
    getProjectInfo () {
      ...
      this.$http.get('/account4a/home/register/acct/acctInfo', { 'params': params }).then(({ data: res }) => {
        ...
        if (res.code !== 0) {
          ...
          let list = res.data
          if (list.hasSubAcct) {
            this.$message.warning('已存在从账号:' + list.loginName)
          } else {
    ...
    submitData (val) {
      ...
      let url
      console.log(55)
      url = '/account4a/home/register/acct/createApplyOrder'
      let data = JSON.parse(JSON.stringify(thisvm.dataForm))
      if (data.isAddOrg === '0') {
        delete data.toCreateOrganizeForm
      }
