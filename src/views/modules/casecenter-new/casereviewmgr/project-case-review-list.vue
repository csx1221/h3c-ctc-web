            ...
            <el-table-column fixed="right" label="操作">
              <template slot-scope="scope">
                <template v-for="(item, index) in scope.row.operate">
                  <el-button v-if="['23','24'].includes(item)" @click="doOperation(item,scope.row)" :key="index" type="text" size="small">{{ $getDictLabel('operateType',item) }}</el-button>
...
export default {
  ...
  methods: {
    ...
    doExportCase () {
      ...
      let url = '/ctc/case/review/exportReviewCase'
