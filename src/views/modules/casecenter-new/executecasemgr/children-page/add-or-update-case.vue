<template>
  <el-dialog :title="getTitle()" :visible.sync="innerVisible" append-to-body :close-on-click-modal="false" width="70%" class="add-or-update-case">
  ...
  methods: {
    ...
    getCaseDataById () {
      ...
      thisvm.$http.get('/ctc/exec/case/' + thisvm.dataForm.caseId).then(({ data: res }) => {
        ...
        thisvm.dataForm.score = thisvm.dataForm.score || '1.0'
        thisvm.dataForm.shouldTest = thisvm.dataForm.shouldTest || '1'
      }).catch(() => { })
    },
    getTaskInfoById () {
