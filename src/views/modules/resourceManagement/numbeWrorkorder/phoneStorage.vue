        ...
        <el-row style="margin-bottom: 20px">
          ...
          <el-col :span="12" align="right">
            <el-button
              type="primary"
              size="small"
              @click="createConfig('新增手机', '9')"
              >增加</el-button
            >
            <el-button
              type="primary"
              size="small"
              @click="createConfig('批量导入', '10')"
              >批量导入</el-button
            >
            <el-button
              type="primary"
              size="small"
              @click="createConfig('批量获取APP测试未入库手机', '11')"
              >批量获取APP测试未入库手机</el-button
            >
            <el-button
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <el-button
                @click="deleteOperate(scope.row, scope.$index)"
                type="text"
                size="small"
                >删除</el-button
              >
...
export default {
  ...
  methods: {
    ...
    doTableOperate () {
      if (this.dataListSelections.length !== 0) {
        ...
        this.dataListSelections.forEach((item, index) => {
          mobilephoneIds.push(item.mobilephoneId)
        })
        this.$http['delete'](`/ctc/resource/activiti/mobilephone/delBatch`, { 'data': mobilephoneIds }
        ).then(({ data: res }) => {
          if (res.code !== 0) {
    ...
    deleteOperate (row, index) {
      console.log(row)
      this.$http['delete'](`/ctc/resource/activiti/mobilephone/del/${row.mobilephoneId}`
      ).then(({ data: res }) => {
        if (res.code !== 0) {
          return this.$message.error(res.msg)
        } else {
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          if (this.dataList.length === 0) {
            ...
            let params = { ...this.form, definitionId: 4, mobilephoneList: this.dataList, taskId: taskId }
            // let mobilephoneList = []
            // for (var k = 0, length = this.dataList.length; k < length; k++) {
            //   mobilephoneList.push(this.dataList[k].mobilephoneId)
            // }
            thisvm.$http['post'](code ? '/ctc/resource/activiti/apply/resubmitMobilePhoneApply' : '/ctc/resource/activiti/apply/addMobilePhoneApply',
              params
            ).then(({ data: res }) => {
              if (res.code !== 0) {
    ...
    createConfig (title, type, data) {
      ...
      if (type === '9') {
        ...
        this.$nextTick(() => {
          this.$refs.addOrUpdata.init(title, '9')
        })
      } else if (type === '10') {
        this.addOrUpdataVisible = true
        this.$nextTick(() => {
