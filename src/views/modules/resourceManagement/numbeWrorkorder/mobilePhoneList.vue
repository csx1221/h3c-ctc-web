...
export default {
  ...
  methods: {
    ...
    getPhoneListData () {
      ...
      this.$http.get('/ctc/resource/activiti/mobilephone/getMobilePhone').then(({ data: res }) => {
        if (res.code !== 0) {
          this.$message.error(res.msg)
        } else {
          this.$http.get('/ctc/resource/activiti/mobilephone/AppPage', { 'params': params }).then(({ data: result }) => {
            if (result.code !== 0) {
              that.$message.error(result.msg)
            } else {
              that.dataList = result.data.list
              that.total = result.data.totalCount
              // 回显已勾选的数据
              if (that.selectData) {
                for (let i = 0; i < that.selectData.length; i++) {
                  for (let k = 0; k < that.dataList.length; k++) {
                    if (that.selectData[i].mobilephoneId == that.dataList[k].mobilephoneId) {
                      that.$nextTick(() => {
                        that.$refs.dataList.toggleRowSelection(that.dataList[k]);
                      })
                    }
                  }
                }
