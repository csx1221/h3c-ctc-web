  ...
  <el-card shadow="never" class="aui-card--fill defect-data-detail">
    ...
    <el-row>
      <el-table stripe highlight-current-row v-loading="dataListLoading" :data="dataList" border @selection-change="dataListSelectionChangeHandle" style="width: 100%;">
        ...
        <!--                <el-table-column prop="organName" label="短信接收源" header-align="center" align="center" show-overflow-tooltip ></el-table-column>-->
        ...
        <!--                <el-table-column prop="organName" label="办卡人联系方式" header-align="center" align="center" show-overflow-tooltip ></el-table-column>-->
        <el-table-column prop="dateState" label="状态" header-align="center" align="center" show-overflow-tooltip>
          <template slot-scope="scope">
            <!--                        <el-popover-->
            <!--                                placement="top-start"-->
            <!--                                title="标题"-->
        ...
        <el-table-column fixed="right" label="操作" header-align="center" align="center">
          <template slot-scope="scope">
            <el-button @click="createConfig('编辑','1',scope.row,scope.$index)" type="text" size="small">编辑</el-button>
  ...
  methods: {
    ...
    createConfig (title, type, data, index) {
      if (type === '1') {
        ...
        this.$nextTick(() => {
          this.$refs.addOrUpdata.init(title, type, data, index)
        })
      } else if (type === '2') {
    ...
    submitData () {
      ...
      // console.log(params)
      thisvm.$http.post('/ctc/resource/cardnumber/saveBatch', { list: this.dataList }).then(({ data: res }) => {
    ...
    initFileImport () {
      ...
      fileImportParams.uploadUrl = '/ctc/resource/cardnumber/importBatch'
    ...
    validateData ($index, item, resolve, reject) {
      let thisvm = this
      let params = { ...item }
      thisvm.$http.post('/ctc/resource/cardnumber/updateCardNumberImport', params).then(({ data: res }) => {
        if (res.code !== 0) {
    ...
    getDataList () {
      ...
      this.$http.get('/ctc/resource/cardnumber/getCardNumberImportList').then(({ data: res }) => {
