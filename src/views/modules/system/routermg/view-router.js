...
export default {
  data () {
    ...
    return {
      ...
      showRoledicDetail: false,   // 查看字典详情的visible状态
      showRouteDetail: false,     // 查看路由管理的详情的visible状态
  ...
  methods: {
    ...
    roledicDetailList (id, num) {
      ...
      this.showRoledicDetail = true
      this.$nextTick(() => {
        this.$refs.roledicDetail.dataForm.id = id
        this.$refs.roledicDetail.dataForm.uuid = num
        this.$refs.roledicDetail.init()
        this.$refs.roledicDetail.active = num
        this.$refs.roledicDetail.displayChild = num
      })
    },
    // 路由管理的传入参数
    routerAddOrUpdateHandle (id, pid, num) {
      if (num === 1 && this.currentRow === null) {
        this.$message.error('请选择项目!')
        return
      } else if (num === 1 && this.currentRow.projectType === '0') {
      ...
      this.$nextTick(() => {
        this.$refs.addOrUpdate.dataForm.applicationId = id
        this.$refs.addOrUpdate.dataForm.id = pid
