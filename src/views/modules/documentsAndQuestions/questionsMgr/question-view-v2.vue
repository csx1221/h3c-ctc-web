        ...
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="计划开始日期：" prop="planStartTime">
              <span>{{ dataForm.planStartTime}}</span>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="计划结束日期：" prop="planEndTime">
              <span>{{ dataForm.planEndTime}}</span>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="上线日期：" prop="onlineTime">
              <span>{{ dataForm.onlineTime}}</span>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="测试人员名称：" prop="testName">
              <span>{{ dataForm.testName}}</span>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="测试用例数量：" prop="testCaseNum">
              <span>{{ dataForm.testCaseNum}}</span>
            </el-form-item>
          </el-col>
          <el-col :span="24">
            ...
            <el-row>
              ...
              <el-col :span="24" v-if="operationHandlerVisible">
                <el-form-item prop="operationHandler" label="处理人名：">
                  <ctc-project-question-handler-v2 v-model="dataForm.operationHandler" placeholder="请选择处理人" :projectId.sync="dataForm.projectId" :echoDataForm.sync="echoDataForm" @watchChildPage="triggerParentPage" style="width:100%;"></ctc-project-question-handler-v2>
                </el-form-item>
              </el-col>
              <el-col :span="24">
          ...
          <el-col :span="24" style="padding-left:35px;margin-bottom:54px;" v-if="(questionTrajectoryData || []).length>0">
            ...
            <el-table v-else :data="questionTrajectoryData" :expand-row-keys="expandRowKeys" :row-key="getRowKeys" @expand-change="expandChange" stripe border>
              ...
              <el-table-column prop="handlerStep" label="下一步流程" header-align="center" align="center" show-overflow-tooltip>
                <template slot-scope="scope">
                  {{  scope.row.handlerStep==-1?'':$getDictLabel('question.handlerStep', scope.row.handlerStep) }}
                </template>
              </el-table-column>
...
export default {
  ...
  methods: {
      ...
      if (this.dataForm.questionId) {
        this.$http.get('/ctc/project/question/' + this.dataForm.questionId).then(({ data: res }) => {
          if (res.code !== 0) {
            ...
            this.echoDataForm.type = this.dataForm.type + ''
            ...
            if (this.dataForm.operationList.length > 0) {
              this.$nextTick(() => {
                this.$refs.ctcFileUpload.customUploadParams = { 'maxCount': 50, 'checkFileType': ['.doc', '.docx', '.xls', '.xlsx', '.txt', '.png', '.jpeg', '.jpg', '.zip', '.rar'] }
    ...
    operationClick (item) {
      let thisvm = this
      // 未解决
      if ([9].includes(item.operation)) {
        let filterHandlerData = []
        thisvm.questionTrajectoryData.forEach((item, index) => {
          //  处理状态(6:待验证)
          if (thisvm.dataForm.authSerialNumber === item.authSerialNumber && [6].includes(item.dealStatus)) {
            filterHandlerData.push(item.creator)
          }
        })
        thisvm.operationHandlerVisible = true
        thisvm.$nextTick(() => {
          // 特殊判断如果没有解决人处理，默认选中总部人员
          if (filterHandlerData.length === 0) {
            thisvm.echoDataForm.type = '-1'
          } else {
            thisvm.echoDataForm.type = thisvm.dataForm.type + ''
          }
          thisvm.echoDataForm.operation = item.operation + ''
          thisvm.echoDataForm.multiSelect = '1'
          thisvm.echoDataForm.filterHandlerData = filterHandlerData
          thisvm.echoDataForm.random = 'clear_' + Math.random()
        })
      } else if ([4].includes(item.operation)) {
        let filterHandlerData = [this.dataForm.creator]
        thisvm.questionTrajectoryData.forEach((item, index) => {
          //  处理状态(6:待验证)
          if (thisvm.dataForm.authSerialNumber === item.authSerialNumber) {
            filterHandlerData.push(item.handler)
          }
        })
        thisvm.operationHandlerVisible = true
        thisvm.$nextTick(() => {
          thisvm.echoDataForm.type = thisvm.dataForm.type + ''
          thisvm.echoDataForm.operation = item.operation + ''
          thisvm.echoDataForm.multiSelect = '0'
          thisvm.echoDataForm.filterHandlerData = filterHandlerData
          thisvm.echoDataForm.random = 'clear_' + Math.random()
        })
      } else {
        this.operationHandlerVisible = false
      }
    },
    doOperationBtn (type) {
      if (type === 1) {
