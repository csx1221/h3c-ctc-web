    ...
    <split-bar>
      <div slot='split_left' style="background-color: white;height: 100%;padding: 20px 10px 0;margin-right: 10px;margin-left: -5px;">
        ...
        <el-tree highlight-current ref="projectTreeData" :data="projectTreeData" :load="loadTreeNode" lazy :props="defaultProps" node-key="id" @node-click="handleNodeClick" :expand-on-click-node="false" style="height:700px;overflow-y:auto;overflow-x:hidden;">
          <span class="custom-tree-node" :title="getTreeIcon('title',data)" slot-scope="{ node, data }" style="display:block;">
            ...
            <span>
              {{data.dirName}}
              <i class="el-icon-edit" v-if="data.ifAdmin==1 && data.dirLevel>-1" @click="doDirModuleEdit(data)" style="margin-left:10px;margin-right:10px;"></i>
            </span>
          </span>
        </el-tree>
        <!--引入目录模块-->
        <dir-module ref="dirModule" :visible.sync="dirModule" @watchChildTree="triggerParentPage"></dir-module>
      </div>
      <el-container slot='split_right'>
        <div style="margin-left: 10px;-webkit-box-flex: 1;box-sizing: border-box;flex: 1 1 auto;overflow: auto;">
          <document-dir-list :projectData="projectData" @watchChildTree="triggerParentPage"></document-dir-list>
        </div>
      </el-container>
    </split-bar>
  </el-container>
...
<script>
import DirModule from './choose-dialog/dir-module'
import DocumentDirList from './document-dir-list'
export default {
  name: 'DocumentDirTree',
  components: { DirModule, DocumentDirList },
  data () {
    return {
      dirName: '',
      projectData: {
  ...
  methods: {
    ...
    handleNodeClick (node) {
      ...
      tnode.pDirId = tnode.pid || tnode.projectId
      tnode.nodePath = this.dirTreeMap[tnode.id] || this.dirTreeMap[tnode.projectId] || {}
