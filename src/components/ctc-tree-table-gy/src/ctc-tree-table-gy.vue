    ...
    <div v-else class="el-input el-input-group el-input-group--append">
      <div class="el-input__inner" v-if="nameList.length>0">
        <template v-for="(item,index) in nameList">
          <el-tag class="eltag" closable @close="handleClose(item)" :key="index">{{item.name}}</el-tag>
        </template>
      </div>
    ...
    <el-dialog :visible.sync="visible" width="35%" :modal="false" :title="title" :close-on-click-modal="false" :close-on-press-escape="false">
      <el-row class="leftTree tree-container">
        ...
        <el-col :span="24" class="tree-data">
          <el-tree v-show="isThis===0" ref="leftTreeData0" :data="data" :props="defaultProps" :default-expand-all="defaultExpandAll" :default-checked-keys="checkedKeys" @check="leftCheckChange" node-key="id" class="treeLight treeLine treeCss" @node-click="handleNodeClick" :show-checkbox="type==='radio'?false:true" highlight-current :check-strictly="false" :indent="3" :expand-on-click-node="false" :check-on-click-node="true" style="height:440px;overflow-y:auto;">
          ...
          <el-tree v-show="isThis===1" ref="leftTreeData1" :data="data" :props="defaultProps" default-expand-all :default-checked-keys="checkedKeys" @check-change="checkChange" node-key="id" class="treeLight treeLine treeCss" @node-click="handleNodeClick" :show-checkbox="type==='radio'?false:true" highlight-current :check-strictly="false" :indent="3" :expand-on-click-node="false" :check-on-click-node="true" style="height:440px;overflow-y:auto;">
            <span class="custom-tree-node" :title="getTreeIcon('title',data)" slot-scope="{ node, data }" style="display:block;">
              <img :src="getTreeIcon('src',data)" style="width:16px;height:16px;margin-right:5px;" />
              <span>{{data.name}}</span>
            </span>
...
export default {
  ...
  methods: {
    leftCheckChange (data, isCheck, isChildrenCheck) {
      if (this.sign === 'testObject') {
        this.checkedKeys = []
        this.leftCheckData = []
        if (isCheck.checkedNodes.length > 0) {
          let checkNodes = isCheck.checkedNodes
          for (let i = 0; i < checkNodes.length; i++) {
            if (checkNodes[i].id) {
              this.checkedKeys.push(checkNodes[i].id)
              this.leftCheckData.push(checkNodes[i])
            }
          }
        }
      }
    },
    checkChange (data, isCheck, isChildrenCheck) {
      if (this.isThis === 1 && this.sign === 'testObject') {
        if (data.id) {
          if (isCheck) {
            ...
            if (this.leftCheckData.indexOf(data) !== -1) {
              this.leftCheckData.splice(this.leftCheckData.indexOf(data), 1)
            }
          }
        }
      }
        ...
        if (this.sign === 'organizeId') {
          ...
          for (let i = 0; i < lists.length; i++) {
            ...
            for (let j = 0; j < lists[i].list.length; j++) {
              nodeList.push({ nodeType: 2, ...lists[i].list[j] })
            }
            node.list = nodeList
            newData.push(node)
          }
          this.isThis = this.dataForm.name ? 1 : 0
          this.data = newData
          if (this.dataForm[this.defaultDataFormProps.name]) {
            this.isPush = true
          } else {
            this.isPush = false
          }
        ...
        this.$http.get(this.baseOption.echoUrl + this.id).then(({ data: res }) => {
          if (res.code !== 0) {
            ...
            for (let i = 0; i < list.length; i++) {
              ...
              handleData.push({ 'projectId': list[i].projectId, 'organizeId': list[i].organizeId, 'organizeType': list[i].organizeType, 'organizeName': list[i].organizeName })
            }
