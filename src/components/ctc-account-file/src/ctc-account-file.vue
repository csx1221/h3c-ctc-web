<template>
  <!--文件导入-->
  <div class="file-import-data file-import-account-dialog">
    <el-row :gutter="0">
      <el-col :span="24">
        <el-button size="small" @click="downFileTemplate">下载模板</el-button>
...
export default {
  ...
  methods: {
    ...
    downFileTemplate () {
      ...
      if (fileImportParams.templateUrlLocal) {
        ctcUtils.customExportLocalGet(fileImportParams.templateUrlLocal, data)
      } else {
    ...
    uploadHandleSuccess (response, file, fileList) {
      this.$emit('watchChildImport', { type: 'reloadTable', 'data': [] })
      if (response.code !== 0) {
