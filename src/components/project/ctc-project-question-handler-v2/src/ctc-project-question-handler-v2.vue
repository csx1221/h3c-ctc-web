<template>
  <div>
    ...
    <el-dialog :visible.sync="visibleDialog" width="25%" :modal="false" :close-on-click-modal="false" :close-on-press-escape="false">
      <el-tree highlight-current ref="projectTreeData" :data="projectTreeData" :default-checked-keys="checkedKeys" :show-checkbox="validMultiSelect()" default-expand-all :props="defaultProps" node-key="id" @node-click="handleNodeClick" :expand-on-click-node="false" style="height:600px;overflow-y:auto;overflow-x:hidden;">
      ...
      <template slot="footer" v-if="validMultiSelect()">
...
<script>
export default {
  name: 'CtcProjectQuestionHandlerV2',
  data () {
    return {
      visibleDialog: false,
      checkedKeys: [],
  ...
  props: {
    ...
    echoDataForm: {
      type: Object,
      default () {
        return { 'random': '', 'multiSelect': '1', 'operation': '0', 'flow': '0', 'type': '0', 'handler': '', 'handlerName': '', 'filterHandlerData': [] }
      }
    },
    disabled: {
      type: Boolean,
  ...
  watch: {
    'echoDataForm.random' (newVal, oldVal) {
      this.echoData()
    }
  },
  methods: {
    ...
    validMultiSelect () {
      let bool = true
      // 判断是否多选模式
      if (this.echoDataForm.multiSelect === '0') {
        bool = false
      }
      return bool
    },
    getSeachTreeData () {
      let thisvm = this
      let projectId = thisvm.projectId || thisvm.parentProjectId || ''
      if (projectId) {
        // 设置默认选中
        thisvm.checkedKeys = thisvm.idsList
        let params = {}
        let url = '/ctc/project/question/listQuestionHandlerV2'
        if (thisvm.echoDataForm.flow === '0') {
          params.parentId = projectId
          url = '/ctc/project/projectParticipant/subProjectSelectableParticipant'
        } else {
          params.projectId = projectId
          params.type = thisvm.echoDataForm.type
        }
        thisvm.$http.get(url, { 'params': params }).then(({ data: res }) => {
          ...
          if (thisvm.echoDataForm.flow === '0') {
            ...
            if (thisvm.echoDataForm.filterHandlerData.length > 0 && res.data.length > 0) {
              let list = res.data[0].list.filter(function (item) {
                // 4:转派
                if (thisvm.echoDataForm.operation === '4') {
                  return !thisvm.echoDataForm.filterHandlerData.includes(item.id)
                } else if (thisvm.echoDataForm.operation === '9') {
                  // 未解决
                  return thisvm.echoDataForm.filterHandlerData.includes(item.id)
                } else {
                  return true
