<template>
  <div>
    <el-input v-model="showOrganizeName" :placeholder="placeholder" readonly>
    ...
    <el-dialog :visible.sync="visibleOrganize" width="45%" :modal="false" :title="placeholder" :close-on-click-modal="false" :close-on-press-escape="false">
      <el-row>
        <el-col :span="16">
          <el-input v-model="searchName" size="mini" placeholder="请输入责任人" clearable />
        </el-col>
      ...
      <el-table ref="tableObject" :data="organizeList" style="width: 100%;height:300px;">
        ...
        <el-table-column prop="realName" label="责任人" header-align="left" align="left"></el-table-column>
      </el-table>
  ...
  methods: {
    ...
    echoOrganizeData () {
      if (this.organizeId !== '' && this.organizeId.indexOf('clear') === -1) {
        this.$http.get(`/ctc/plan/demand/liable?id=${this.organizeId}`).then(({ data: res }) => {
          if (res.code !== 0) {
            return this.$message.error(res.msg)
          } else {
            this.showOrganizeName = res.data.data.list[0].realName
          }
        }).catch(() => { })
      } else {
        this.showOrganizeName = ''
    ...
    getOrganizeList () {
      return this.$http.get('/ctc/plan/demand/liable', {
        params: {
    ...
    commitHandle () {
      ...
      this.$emit('update:organizeName', node.realName)
      this.$emit('watchChildPage', { 'type': 'chooseOrganize', 'data': node })
      this.showOrganizeName = node.realName
      this.visibleOrganize = false
      this.organizeList = []
    }
  }
